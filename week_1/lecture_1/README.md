...
* Alexa responds to queries using ML
* George Hotz is using ML to win level 3 in three years
* A program called AlphaGo used ML to beat the world champion at Go


## Whats the definition of ML

```
	"Machine Learning as a discipline aims to design, understand and apply computer programs to learn from experience(ie, data) for the purpose of modeling, prediction or control
```
...
There are many types of problems that ML can tackle:

### Future Events

<img src="img/img1.png" alt="future events  img" width="300"/>

* We might want to predict the price of a stock/index/derivative based on the price pattern up untill now. If we want to know the price one week, month, year into the future, ML can help with that. 

* We can also use ML to find out if a diesese will return or the likelihood of it spreading/getting worse.

* A self driving car would use ML to judge what the traffic around it might do to avoid accidents or simply to stay in lane and abide by the local laws.


### About properties we don't know

* Based on the preference of a viewer (the movies he/she saw) we can use ML to recommend movies the user might be interested in to watch next, and not show ones the user might dislike.

* We can predict whether a compound might be soluble in water or not.

* We can train a model to tell us what an image is of or what it contains
...
For the beginning section of this course we will study supervised learning.
```
Wikipedia Says:

	Supervised learning is the machine learning task of learning a function that maps as
	input to an output based on an example input to an output based on example input-output pairs

	It infers a function from labeled training data consisting of a set of training examples 

```

What this definition means will be clear eventually, but lets looosely look at what the general idea is.


### A loose example: Supervised Learning

Suppose we want to train a model to to cassify an image. What this means is we will feed the model an image and it will make a prediction of what it thinks the image is of.
...
The hardcoded engineering way of accomplishing the image classification problem might be to provide specific rules for each category. For instance if we have an image of a mushroom

<img src="img/img2.png" alt="mush img" width="200"/> 
...
But no matter how many ridged rules we specify, a sightly different image of a mushroom will violate the rules and it will be misclassified.

For example; Here we clearly see two/three mushrooms, but the machine will not be able to detect this if it is following ridged rules. 

<img src="img/img3.png" alt="mush2 img" width="200"/> 
...
* We will train its parameters, 𝚯, with many correct examples of proper translation.
...
From this example we will see how the mathmatical notions are written and how the problem is formulated mathmatically.


#### The problem

A user is recommended movies to watch based on the movies they had watched earlier. We want a binary answer from our algorithm (model) i.e we want to know, for any given movie, if the user will prefer to watch it (YES) or not (NO)

#### Traning set

As dicussed earler the model will learn from correct examples that we feed it. Here for the sake of simplicity we have selected data for the following four movies.

<img src="img/img4.png" alt="movies img" width="500"/>
...
Maybe the viewer did a survay or maybe the viewer watched the +1 movies and scrolled passed the -1 movies and Netfix recorded their prefrences.

The point is we have labels for movies that reflect viewers preferences correctly.

#### Feature vector: 𝓧⁽ⁱ⁾

Now we plan to predict the label of a new movie therefore concluding whether the viewer will be recommended it or not.
...
```
	...
	𝓧 = | Comedy? | --> The first element will be 1 if it's a comedy, 0 otherwise
	    ...
	    |    ┇    |
	    |   Imdb  | --> The last element will be a numerical imdb rating of the movie
	    --       --
```

The features will be chosen by us depending on avaliability of data, importance of feature and complexity of the problem.
...
It is to note that depending upon the formation of our problem we can see that each element can be binary, discrete or continuous.

But more generally we write:
```
	𝓧 ⋲ ℝᵈ equivalently 𝓧⁽ⁱ⁾ ⋲ ℝᵈ

	(meaning 𝓧 in in the set of real numbers to the ᵈ dimension; ℝᵈ)
```
...
```
	...
	𝓧⁽ⁱ⁾⋲ ℝᵈ: Feature vector of the ith movie.	With each movie labled correctly

	𝕐⁽ⁱ⁾: Label of the ith movie
	⋲ {+1, -1}
```

And with this data we plan to teach/train the machine mathmatically.

The traning set will be denoted by Sn, where the subscript 'n' tells us the number of cases we have in the set.
...
Lets now move on to first visually see whats happening before we introduce some new terminology
...
Each movie will still have the label 𝕐⁽ⁱ⁾ which will be denotted by
```
	+ : if the label is +1
	- : if the label is -1
```
...
<img src="img/img5.png" alt="movies img" width="500"/>
...
And for our example 4 points are drawn because n=4, ie thenumber of cases in the traning set is equal to 4.
...
Now as we decided / discussed earlier, we plan to predict whether the viewer will prefere to watch the movie, in which case it will be labelled +1 or prefere not to watch the movie, in which case it will be labelled -1

This will be done on a new set of movies disjointed from our traning set (because we cant test on data it has already seen as it will know the answer, thus making the traning useless)
...
<img src="img/img6.png" alt="movies test set img" width="400"/>
...
We will use a function also called a mapping, also called a classifier. Here we will refewr to it as h(), it will take as input the feature vector and spit out the label +1 or -1 according to some rule. 

This is the most general and obvious thing we can do. How do we choose h(), we'll see later but for now just understand a classifier is a function/mapping that takes elements (vectors) from the space in which features vectors live, which we denote here by 𝓧 and maps it into the categories or label space, here it's {-1,1}
```
	h : 𝓧 --> {-1, 1}
``` 
...
The blue region is where our classifier labels the 𝓧 vectors inside the region as +1, ie h(𝓧) = +1 or h(𝓧) = -1 in the white region.

Here are some examples

<img src="img/img8.png" alt="Classifier img" width="500"/>


Now look at the following two classifiers

<img src="img/img7.png" alt="Classifiers img" width="500"/>
...
Why would we want such a classifier that labels 2 out of 4 movies wrongly when we already know their correct labels.
...
We need a quantity that can be measured to the traning set and that is what we will study next.



### Traning Error En(h)

The idea is pretty simple. we want to see what fraction of points in the traning set does our classifier h, classify incorrectly. we are taking points from the traning set so we:
* Have each point / vector with the label  𝕐⁽ⁱ⁾
* we will commute what our classifier predicts for  𝓧 ⁽ⁱ⁾, ie we find h(𝓧 ⁽ⁱ⁾)
* If h(𝓧 ⁽ⁱ⁾) ≇ 𝕐⁽ⁱ⁾, then the classifier committed an error
...
This will give us an idea of the classifiers performance with the traning data set.
...
<img src="img/img9.png" alt="is data correct? img" width="700"/>

<img src="img/img10.png" alt="Is data correct? img" width="700"/>
...
<img src="img/img11.png" alt="Classifiers img" width="600"/>

All three of them has a traning error equal to 0. But we have a problem here. To see that lets look at the 3rd case 

<img src="img/img12.png" alt="3rd case" width="600"/>
...
The blue region of our classifier above is so narrow that only the feature vectors that are very close (in the bear vicinity) of the traning data examples will be labelled +1, all the others will fall into the white region and therefore be labelled -1.

For our example above, the '?' represents points / vectors from the test set. None of them fall into the blue region due to it's narrowness. we don't have the labels of these examples beforehand, so it's a fair bet to assume that half of them will be a +1 (label) in reality and half of them will be a -1 (label) in reality.
...
<img src="img/img13.png" alt="Classifier" width="500"/>


No matter how many points we have in our traning set. We can always find a mapping h() that will wrap a small circle around my +1 vectors for every +1 labeled vector in the traning set. This will then give me a classifier h() which will have the traning error En(h) = 0 always. but this will again lead us to a coin flip prediction for each test data. This is because we were free to choose any classifier that we liked.
...
Now in the next lecture we will continue our journey to explore the classifier and see how we constrain our classifier to a family of linear functions. But before that we need to see that this idea of function h() can be used to solve many problems like

<img src="img/img14.png" alt="img" width="500"/>

<img src="img/img15.png" alt="img" width="500"/>
